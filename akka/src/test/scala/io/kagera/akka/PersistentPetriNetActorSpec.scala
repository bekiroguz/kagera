...
class PersistentPetriNetActorSpec extends TestKit(ActorSystem("test", PersistentPetriNetActorSpec.config))
  ...
  "A persistent petri net actor" should {
    ...
    "Respond with a TransitionFailed message if a transition failed to fire" in new StateTransitionNet[Set[Int], Event] {
    ...
    "Respond with a TransitionNotEnabled message if a transition is not enabled" in new StateTransitionNet[Set[Int], Event] {
    ...
    "Retry to execute a transition with a delay when the exception strategy indicates so" in new StateTransitionNet[Set[Int], Event] {
      ...
      val t1 = transition(id = 1, exceptionStrategy = retryHandler) {
        set ⇒ { throw new RuntimeException("something went wrong") }
      }
