...
object PersistentPetriNetActorSpec {
  ...
  def stateTransition[S, E](eventSourcing: S ⇒ E ⇒ S, fn: S ⇒ E, id: Long = Random.nextLong, label: String = "", isManaged: Boolean = false): Transition[Unit, E, S] =
    new AbstractTransition[Unit, E, S](id, label, isManaged, maximumOperationTime = Duration.Undefined) with UncoloredTransition[Unit, E, S] {

      override val updateState = eventSourcing

      override def produceEvent(consume: Marking, state: S, input: Unit)(implicit executor: ExecutionContext): Future[E] = Future { (fn(state)) }
    }
...
class PersistentPetriNetActorSpec extends TestKit(ActorSystem("test", PersistentPetriNetActorSpec.config))
  ...
  "A persistent petri net actor" should {
    ...
    "fire automatic transitions in paralallel when possible" in {
      ...
      val actor = system.actorOf(Props(new PetriNetProcess[Unit](petriNet, initialMarking, ())), java.util.UUID.randomUUID().toString)
